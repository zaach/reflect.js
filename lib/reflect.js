...
exports.compile = function (source, link) {
    var b = require("reflect-tree-builder");
    var transform = require("./transform").transform;
    var ast = exports.Reflect.parse(source, {builder: b});

    var dependencies = transform(ast, b);
    var target = stringify(ast);

    if (link) {
        target = read(path.join(__dirname,"es6.js")) + target +"\n";
        Object.keys(dependencies).sort().forEach(function (dep) {
            target += read(path.join(__dirname, "append", dep+".js"));
        });
    }
